...
| Revision (flashcards) | done — `/prep/revise`, Anki-style. Decks are derived, never stored, using the same `isQuestion` rule as Question lists + search below (a fixed prefix-based rule shipped in the original PR wrongly assumed LLD questions sat under `lld/questions/`, mirroring HLD; LLD actually has no such folder, so the deck showed 0 LLD cards until this was caught in production and switched to reuse the shared, tested classifier). All, DSA, HLD, LLD, and per company All/DSA/HLD/LLD from the rows on the company's generated DSA/HLD/LLD index pages (titles matched to question pages the same lenient way the index links them; unmatched titles are listed under the deck, not dropped). A session is the deck shuffled server-side, one card at a time: front = name + prompt + (DSA) problem summary and example; Show answer (space) reveals the rest — the full `DsaSolution` minus the problem, or the page body, answer fields, notes and code. Again/Hard/Good/Easy (keys 1–4) with Anki-style interval labels; Again requeues the card to the back of the session. Progress bar, done/left/coming-back counts, per-rating tally, card and session timers. Scheduling is a simplified SM-2 in `domain/revision.ts`, state in `prep_reviews`, keyed by `(deck_id, prep_item_id)` (migration 0022) rather than by card alone — decks overlap by design (Everything is the union of every other deck; a company deck shares cards with its discipline deck), so progress must be independent per deck or resetting one deck would silently reset every other deck built from the same cards, which is exactly the bug this migration fixed. Each deck card that has any progress shows a "Reset progress" action (a native confirm, then deletes only that deck's review rows — never a sibling deck's). Rating and reset both call `revalidatePath("/prep/revise")`, needed because Next's client Router Cache reuses a page's RSC payload on browser back/forward regardless of `force-dynamic`; without it, hitting Back after a session showed stale counts until a manual refresh. |
| Question lists + search | done (2026-09-24) — `/prep/dsa`, `/prep/behavioral`, `/prep/system-design` and every non-company folder page (`…/hld`, `…/lld`, a DSA pattern) list *every* question in the subtree at any depth, not just direct children. A question is any page that is not a folder, a `notes` page or a reader; in system design only `hld/questions/*` and `lld/*` outside `lld/resources` count. Searched client-side (`server/domain/prep-questions.ts`): word-order-free AND match across title, topics, folder names, companies and prompt; prefix-as-you-type, typo tolerance, space-free matching ("ratelimiter") and shorthand (`dp`, `ll`, `bst`, `dsu`, …). Difficulty/status chips, `/` to focus, query mirrored to `?q=`. |
| Custom revision decks | done (2026-09-25) — `/prep/revise/new`, a "Your decks" section on `/prep/revise`. Filter by company/discipline/difficulty (each optional) and either save the filter as-is (a live deck: a question published later that matches shows up with no edit — `domain/revision.ts`'s `buildCustomDeck` looks up the matching built-in company/discipline deck and filters by difficulty on top, rather than re-implementing matching) or check off specific questions from the filtered list to freeze the deck to exactly those ids (a later match is never added). New `prep_custom_decks` table (migration 0023); a custom deck's id is `["custom", <row id>]`, so it gets independent per-deck progress, its own Reset action and correct back-nav behavior for free from the existing deck-progress model — no special-casing needed there. Delete removes the deck and its progress. |
